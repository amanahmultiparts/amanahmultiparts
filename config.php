    ...
    $url = "http://localhost/Amanah%20Multi%20Partss/";
...
if (isset($_COOKIE['login_pimpinan'])) {
    $key_login_pimpinan = $_COOKIE['login_pimpinan'];
    $ciphering_pimpinan = "AES-128-CTR";
    $iv_length_pimpinan = openssl_cipher_iv_length($ciphering_pimpinan);
    $options_pimpinan = 0;
    $decryption_iv_pimpinan = '1234567891011121';
    $decryption_key_pimpinan = "pimpinan_ecommerce"; // Ganti dengan kunci dekripsi Pimpinan
    $decryption_pimpinan = openssl_decrypt($key_login_pimpinan, $ciphering_pimpinan, $decryption_key_pimpinan, $options_pimpinan, $decryption_iv_pimpinan);

    $iduser_key_login_pimpinan = explode("hcCTZvFLD7XIchiaMqEka0TLzGgdpsXB", $decryption_pimpinan);
    $id_user_login_pimpinan = $iduser_key_login_pimpinan[0];

    $select_profile_pimpinan = $server->query("SELECT * FROM `akun` WHERE `id`='$id_user_login_pimpinan' ");
    $profile_pimpinan = mysqli_fetch_assoc($select_profile_pimpinan);
    if ($profile_pimpinan['tipe_akun'] == 'Pimpinan') {
        $akun_pimpinan = 'true';
    } else {
        $akun_pimpinan = 'false';
    }
